...
class PersonalMemoryStore:
    ...
    def init_db(self) -> None:
        with self._connect() as conn:
            # 正式记忆表：存放经过AI提炼的长期记忆
            # status: active-有效 supercede-被替代 archived-过期
            conn.execute(
                """
    ...
    async def hybrid_retrieve(self, query: str, top_k: int = 5) -> list[dict[str, Any]]:
        # 1. 获取两路检索的原始结果
        vector_results = await self._get_vector_search(query, limit=top_k * 2)
        bm25_results = await self._get_bm25_search(query, limit=top_k * 2)
        ...
        for rank, (m_id, _) in enumerate(vector_results, start=1):
            rrf_scores[m_id] = rrf_scores.get(m_id, 0.0) + 1.0 / (k_rrf + rank)

        # 处理 BM25 路结果: list of (memory_id, score)
        for rank, (m_id, _) in enumerate(bm25_results, start=1):
            rrf_scores[m_id] = rrf_scores.get(m_id, 0.0) + 1.0 / (k_rrf + rank)
        ...
        for m_id, _ in sorted_ids:
            memory = self.store.get_memory(m_id)
            if memory:
