...
def get_miniclaw_home_path() -> Path:
    """Get the miniclaw data root, overridable with miniclaw_HOME."""
    configured = os.getenv("miniclaw_HOME")
    if configured:
        return expand_path(configured)

    legacy_home = Path.home() / ".miniclaw"
    migrated_home = Path.home() / "run" / ".miniclaw"
...
def get_data_path() -> Path:
    ...
    return ensure_dir(get_miniclaw_home_path())
...
def get_workspace_path(workspace: str | None = None) -> Path:
    ...
    Args:
        workspace: Optional workspace path. Defaults to <miniclaw_home>/workspace.
    ...
    if workspace:
        ...
        path = get_miniclaw_home_path() / "workspace"
